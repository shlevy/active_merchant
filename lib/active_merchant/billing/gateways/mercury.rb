  ...
  module Billing #:nodoc:
    class MercuryGateway < Gateway
      ...
      URLS = {
        ...
        :production => 'https://w1.mercurypay.com/ws/ws.asmx'        
      }
      ...
      def commit(action, request)
        ...
        File.open('/tmp/debug.txt', 'w') {|f| f.write(endpoint_url) }        
        http_response = ssl_post(endpoint_url, request, build_header)         
        response = parse(action, http_response)

         File.open('/tmp/m_request.txt', 'w') {|f| f.write(request) }

         File.open('/tmp/m_response.txt', 'w') {|f| f.write(http_response) }
